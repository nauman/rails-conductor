...
class ConductorReadTool
  ...
  DEFINITION = {
    ...
    description: "Read-only fleet visibility. Set `action` to one of: " \
      ...
      "edge (read-only inspection of the app's configured Caddy or kamal-proxy edge — app_id/app_name), " \
    ...
    input_schema: {
      type: "object",
      properties: {
        action:          { type: "string", enum: %w[fleet_status server situation logs app_logs deployment transfer cloudflare edge], description: "Which read to perform" },
