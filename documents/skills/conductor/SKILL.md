...
| `conductor_runbook` | `get`, `set_runbook`, `add_item`, `remove_item`, `check_item`, `reset`, `list_rituals`, `get_ritual` | Per-app deploy runbook + checklist, **and** the shared ritual library. **Read (`get`) before deploying** — each app deploys differently. `get_ritual` dereferences the `recipe_id` a finding cites; `list_rituals` shows the library and which rituals an operator has customised. |
...
3. **Standing up an app that does not exist yet?** `conductor_app action: readiness` first — it names the credentials still missing and how to supply each one. **Never ask the operator to paste a secret into the conversation, and never put one in a tool call**; the routes it returns exist so the value does not enter a transcript. Then `conductor_runbook action: get_ritual, recipe_id: "onboard-new-app"`. This file is the *tool surface* — it tells you what you can call, never the order or what is easy to get wrong. The ritual carries the things that have actually bitten: the edge is a property of the server, not the deploy method; a credential belongs in the secret list (in the clear list, kamal inlines it into the `docker run` line, where it reaches logs); provision with `app_id` and no `name` so the convention picks it.
4. **Before deploying an app**, `conductor_runbook action: get` (or read the runbook + checklist from `conductor_read action: deployment`) — each app deploys differently. Work the checklist as you go (`check_item`), and `reset` it before a fresh deploy.
5. After `conductor_app action: deploy`, poll `conductor_read action: deployment` (pass `deployment_id`, or `app_id`/`app_name` for the latest) to confirm success.
...
A finding that carries a `recipe_id` is pointing at a procedure written from a real
incident — fetch it with `get_ritual` rather than improvising. That citation is the
whole point of ADR 0007, and until recently nothing on this channel could follow it.
