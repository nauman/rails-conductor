...
| Retire per-app deploy scripts — every app deploys through Conductor | Runtime Backends | **P0** | M | **Spec'd 2026-08-03** — some apps deploy via repo scripts Conductor never sees, so it holds no release record and can offer them no rollback. A stale record taken as a release baseline once produced a wholly fabricated migration protocol. `ReleaseDriftDetector` now SURFACES this (`unrecorded`); this slot removes the cause. Unblocked by ADR 0003. Also closes the root-SSH gap. |
