...
| Architecture | `docs/architecture/` | Cross-cutting design notes too long for an ADR — `derived-state.md` (what Conductor caches, what refreshes it, where the gaps are) and `rituals.md` (the recipe library, and why a cited ritual is not yet fetchable) |
