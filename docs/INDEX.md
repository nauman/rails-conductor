...
| Architecture | `docs/architecture/` | Cross-cutting design notes too long for an ADR. New pages are numbered folders (`01-builder/` — where an image is built, who decides, and what bounds its cost; `02-secrets/` — what Conductor holds, how it reaches a container on each deploy path, and what none of it buys); earlier ones keep their names so inbound links survive: `derived-state.md` (what Conductor caches, what refreshes it, where the gaps are), `rituals.md` (the recipe library and how a cited ritual is fetched), `database-conventions.md` (how a database is named and reached) |
