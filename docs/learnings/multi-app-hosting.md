...
> Captured from deploying real apps (Conductor, app-one, app-three) onto one shared
...
- **Missing `master.key`** — regenerating creates a new key (and drops old encrypted credential values); store keys centrally (we use the `my-vault` localvault, `<APP>.master_key`).
