...
Tool definitions (names, descriptions, input schemas) come from `GET /mcp/list`, generated by `ToolRegistry` — always trust `/mcp/list` over this table. The surface is **nine flat tools**, each taking an `action` plus that action's params (fewer tools keeps agent tool-selection accurate):
