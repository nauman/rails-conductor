...
| [Operability gaps: resolving failures · schema state · easy backups · per-app schedules · job control](../conductor/plans/07-operability-gaps.md) | Fleet Control | **P0** | M | **Spec'd 2026-07-29** — found by designing the app/fleet/jobs pages against real data. Includes the *live* schema-state probe the row above still lacks, plus the per-app checklist (one-click defaults, exposed over MCP). Mockups: kuickr `conductor/design` |
