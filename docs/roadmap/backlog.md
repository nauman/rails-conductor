...
- **Operability gaps found by design** — **spec 2026-07-29**: designing the app/fleet/jobs pages against real fleet data surfaced five gaps worth fixing together — deploy failures that never resolve (11 failures this week, only 3 real), no schema/seed state between deploys (cause of two prod 500s), backups off on all 12 apps because setup asks for four fields, host-scoped cron instead of per-app in-container schedules, and job counts with no retry/discard. Spec: [../conductor/plans/07-operability-gaps.md](../conductor/plans/07-operability-gaps.md); mockups in kuickr `conductor/design`.
